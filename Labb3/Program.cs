...
switch (args[0])
{
    ...
    case "-new":
        if (args.Length >= 4)
        {
            string currentPathNew = Path.Combine(pathToLabb3Folder, args[1]);
            ...
            if (!File.Exists(currentPathNew))
            {
                ...
                {
                    byte[] buffer = new UTF8Encoding(true).GetBytes($"{string.Join(";", args.Skip(2))}");
    ...
    case "-add":
        ...
        void add()
        {
            string currentPathAdd = Path.Combine(pathToLabb3Folder, args[1]);
            Console.Clear();
    ...
    case "-remove":
        ...
        if (args.Length >= 4)
        {
            string currentPathRemove = Path.Combine(pathToLabb3Folder, args[1]);
            ...
            try
            {
                ...
                {
                    if (args[2] == removeWord.Languages[i])
                    {
                        currentLanguage = i;
                    }
                }
                ...
                {
                    if (removeWord.Remove(currentLanguage, args[j]))
                    {
                        Console.WriteLine($"Your words {args[j]} have been removed");
                        removeWord.Save();
                    }
                    else
                    {
                        Console.WriteLine($"{args[j]} does not exist in this list");
                    }
                }
    ...
    case "-words":
        ...
        {
            string currentPathWords = Path.Combine(pathToLabb3Folder, args[1]);
            ...
            try
            {
                ...
                else
                {
                    ...
                    {
                        if (args[2] != sortList.Languages[i])
                        {
                            currentLanguage = i;
                        }
                    }
    ...
    case "-count":
        ...
        if (args.Length == 2)
        {
            string currentPathCount = Path.Combine(pathToLabb3Folder, args[1]);
            try
            {
    ...
    case "-practice":
        ...
        if (args.Length == 2)
        {
            string currentPathPractice = Path.Combine(pathToLabb3Folder, args[1]);
